...
# @when('search payment in other section by filters Card Entry Mode "{Card_Entry_Mode}", Security Type "{Security_Type}", ISO Result Code "{ISO_Result_Code}", WLP FO Result Code "{WLP_FO_Result_Code}"')
# def step_impl(context, WLP_FO_Result_Code, Card_Entry_Mode, Security_Type, ISO_Result_Code):
...
@when('search payment in other section by filters MCC "{mccIDDropDown}", Acquirer BIN "{targetProviderId}"')
def step_impl(context, mccIDDropDown, targetProviderId):
...
# def step_impl(context, Acq_Cntry_Code, Alt_PAN):
...
def step_impl(context):
    ...
    time.sleep(5)
    Other_payments_search_result = context.driver.find_element_by_xpath("//tr[@class='(blank) t-first']//td[@class='cardPan'][contains(text(),'987178XXXXXX7714')]").text
